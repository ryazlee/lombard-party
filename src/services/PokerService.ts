...
export class PokerService {
  ...
  getPlayerStats(sessions: PokerSession[]): PlayerStat[] {
    ...
    sessions.forEach((session) => {
      ...
      playerMap.set(session.player, {
        ...
        highestSingleWinning: Math.max(existing.highestSingleWinning, session.profit),
      })
    })
    ...
    return Array.from(playerMap.entries()).map(([player, stats]) => ({
      ...
      roi: stats.totalBuyIn > 0 ? (stats.totalProfit / stats.totalBuyIn) * 100 : 0,
    }))
  }
  ...
  filterSessionsByYear(sessions: PokerSession[], year: YearFilter): PokerSession[] {
    if (year === 'all') {
      return sessions
    }
  ...
  async getPlayerYearInReview(snakeCaseName: string): Promise<PlayerYearInReview | null> {
    ...
    const userSessions = sessions.filter(
      (session) => convertNameToSnakeCase(session.player) === snakeCaseName,
    )
    const winningSessions = userSessions.filter((session) => session.profit > 0)
    const winRate =
      userSessions.length > 0 ? (winningSessions.length / userSessions.length) * 100 : 0
